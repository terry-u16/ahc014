...
メモ：logとかsqrtとか取った方が誤差が小さくなる？

bowwowforeachさんが77.70Mを叩き出している。うーん厳しいな。こちらは76.13Mで、さすがにネタが尽きてきた……。他にもレッドコーダー勢が怒濤の追い上げを見せておりヤバい。セーフティリードとか存在しなさそう。Rafbillさんがずっと提出していないのも不気味。

昨日ぶん回したデータで学習させて適用してみたが、微妙。元に戻した。

再加熱、やってみたけど微妙だった。ボツ。

再構築時に長方形候補のsamplerを使い回してロールバックしているが、普通にcloneした方が良いのでは？ → 速くなったのにスコアが下がった。なぜ……。

Cloud Run 1024ケースだと現状1.55M/caseくらい。実行時間を1.2倍にすると1.57M/case、2倍にすると1.61M/caseくらいまでは伸びる。もう一回高速化できないか検討してみる。

プロファイリングすると、ある点から接続できる点を8方向に探しに行くところで時間が取られている。ベクトルを回転させて座標変換→テーブルを引いてbit scan forwardで行っている。ここはかなり削っておりこれ以上の高速化は厳しいので、他の方法を考える。

ある点から既に辺が生えているとき、その方向には絶対に辺を伸ばせない。点ごとに8bitで辺が伸びているかどうかを保持し、既に伸びているなら打ち切ることにする。比較が1回増えるが、密なケースだと恩恵が得られるのでは。

実装。N, Mが大きいケースだと2割くらい高速化された。逆に疎なケースだとトントン～若干悪化くらい。まあスコア伸びたし良いでしょう。

破壊領域の微調整。なんもわからん。

この辺で10000ケース回す。1.562M/caseくらい。提出すると7.631M点。いやキツいな。

いろいろ変わったのでもう一回学習用データを取る。今日はここまで。

## 15日目（土）

メモ：seed=5以外のプロファイリング

メモ：日記を読み直す

メモ：再構築をDFS化

昨日から回していた学習用データが19万件になったので重みを更新する。あまり変わらんかなあ。

乱択貪欲の代わりに乱択DFSも試してみる。あんまり上手く行く気はしないけど。とりあえず雑に実装したところ、0.1M/caseくらい悪化した。まあボツだよね。

ちょっとコードを綺麗にした。行列の演算は今まで普通にfor文でやっていたけれど、代わりにnalgebraというクレートを使うようにした。ちょっと速くなったっぽい。まあもともとここは時間かけてないんだけど、速いと精神衛生上よい。

かなり雑巾を絞りきってもうカラカラになっている。やることなくなったので10000ケース回す。1.567M/case。昨日より良くなっているけれど、誤差の範囲内かもしれない。まあこれを信じることに。

13:20に提出。76.56M。これが限界かな。眠いので一旦寝る。

起きた。Aquariusさんの追い上げが怖いがやることが思い浮かばないので、一昨日発売されたばかりの[リトルウィッチノベタ](https://www.littlewitchnobeta.com/ja/)をプレーする（最悪）。アーリーアクセス版からいろいろと修正されているな。

ynasuさんが76.77Mを出してきて3位転落。このパターンAHC008でもやったな？まあもう手がないし、システスに賭けるしかないな。

## 終了後

解説放送を見た。ランダムプレイアウト部分、もっと色々考えるべきポイントがあったらしい。反省だなあ。N個の候補の中から乱択貪欲するとき、k個拾ってきて一番いいやつを選ぶといいらしい。なるほど……。

システス結果は3,102.04Mで3位フィニッシュ。まあ悔いはないかな。レーティングも+50されて2890。自分にしては上出来。

## 最後に

[リトルウィッチノベタ](https://www.littlewitchnobeta.com/ja/)、ノベタちゃんがかわいいので皆さんやりましょう。
